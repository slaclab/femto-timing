...
class drift_correction():
    ...
    def correct(self):
        ...
        self.flt_pos_ps = self.atm_err_flt_pos_ps_pv.get(timeout = 1.0)  # initial error - COMMENT THIS LINE IF NOT TESTING
        ...
        while (self.count < self.sample_size):
            ...
            if (self.atm_err0 > self.ampl_min) and (self.atm_err0 < self.ampl_max) and (self.atm_err4 != self.flt_pos_ps):  # COMMENT THIS LINE IF NOT TESTING
                ...
                #self.flt_pos_ps = self.atm_err[4] + self.flt_pos_offset  # COMMENT THIS LINE IF TESTING
                self.ampl = self.atm_err0  # COMMENT THIS LINE IF NOT TESTING
                self.flt_pos_ps = self.atm_err4 + self.flt_pos_offset  # COMMENT THIS LINE IF NOT TESTING
