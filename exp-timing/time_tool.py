...
class time_tool():
    def __init__ (self, sys='NULL'): 
        ...
        self.drift_correct_pv[0] = dev_base+'drift_correct_sig'
        for n in range(0,9): # loop over pvs to create
        ...
        self.drift_correct[self.nm[10]] = [Pv(self.drift_correct_pv[0]), Pv(self.drift_correct_pv[0]+'.LOW'), Pv(self.drift_correct_pv[0]+'.HIGH'), Pv(self.drift_correct_pv[0]+'.DESC')]
        for x in range(0,4):
        ...
        self.W = watchdog.watchdog(self.matlab_pv[self.nm[0]][0]) # initialize watchdog   
        ...
        if ( self.ipmpv.value > self.matlab_pv['ipm'][1].value ) and ( self.ipmpv.value < self.matlab_pv['ipm'][2].value ):
             ...
             if ( self.matlab_pv['amp'][0].value > self.matlab_pv['amp'][1].value ) and ( self.matlab_pv['amp'][0].value < self.matlab_pv['amp'][2].value ):
                 ...
                 if ( self.matlab_pv['fs'][0].value != self.old_values['fs'] ) and ( self.matlab_pv['Stage'][0].value == self.old_values['Stage'] ):
                     ...
                     self.drift_correct['dcsignal'][0].put(value = self.matlab_pv['fs'][0].value, timeout = 1.0)
                 #else:
